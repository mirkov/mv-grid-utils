;; Mirko Vukovic
;; Time-stamp: <2011-02-10 07:20:35 grid-functions-unit-tests.lisp>
;; 
...
(defparameter *complex-vector*
  (grid::make-grid `((,*array-type*) ,*complex-type*)
		    :initial-contents '(#c(1d0 2d0) #c(5d0 -3d0))))
...
(defmacro warn-missing-component (definitions tests)
  "Compare contents of function `definitions' and `tests' and flag
missing definitions or tests"
  `(let ((missing-defs (set-difference
		       (mapcar #'first ,tests)
		       (mapcar #'first ,definitions)
		       :test 'equal))
	 (missing-tests (set-difference
		       (mapcar #'first ,definitions)
		       (mapcar #'first ,tests)
		       :test 'equal)))
    (when missing-defs
      (warn "Function tests ~a in ~a are missing corresponding function definitions in ~a" missing-defs (symbol-name ',tests) (symbol-name ',definitions)))
    (when missing-tests
      (warn "Function definitions ~a in ~a are missing corresponding function tests in ~a" missing-defs (symbol-name ',definitions) (symbol-name ',tests)))))


(eval-when (:compile-toplevel :load-toplevel :execute)
  (warn-missing-component *one-arg-functions* *one-arg-func-test-def*))


(defparameter *two-arg-func-test-def*
  `((expt . ((gs . (,*+1-vector* 2d0))
	     (sg . (2d0 ,*+1-vector*))
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (warn-missing-component *two-arg-functions* *two-arg-func-test-def*))
...
(defgeneric cl-calc-2 (fun arg1 arg2)
  ...
  one or both as necessary.  Vector arguments are of grid type in
  sbcl, and native type in clisp.  The return values is a native cl
  array.")
  (:method (fun
	    (arg1 #+sbcl vector-double-float
		  #+clisp vector)
	    (arg2 #+sbcl double-float
		  #+clisp float))
  ...
  (do-fundef (*two-arg-functions*)
	...
	(destructuring-bind (type arg1 arg2) test-def
	  ...
	  (let ((res-cl (cl-calc-2 fun arg1 arg2))
		(res-grid (funcall (grid-fun-name fun) arg1 arg2)))
