(defpackage :tridiag-inverse2
  (:use :cl :gsll :array-indexing2 :lisp-unit :anaphora :grid)
  (:import-from :iterate
		:iter :for))

(in-package :tridiag-inverse2)
  ...
  (let ((n (dim0 matrix)))
    ...
    (iter
     ...
     (iter
      ...
      (with-_-indexing1 (((matrix 0 0)) maref)
...
with theta_0 = 1, theta_1 = a_0"
  (let ((theta (gsll:make-marray 'double-float :dimensions (1+ N))))
    (with-_-indexing1 (((theta 0) (a -1) (b -1) (c -1)) maref)
      (setf theta_0 1d0
	    theta_1 a_1)
...
(defun tridiag->d/e/f (matrix)
  (labels ((cl-vec->marray (cl-vec)
	     (make-marray 'double-float
			  :initial-contents
			  (coerce cl-vec 'list))))
  (values (cl-vec->marray (diagonal (cl-array matrix)))
	  (cl-vec->marray (diagonal (cl-array matrix) :offset 1))
	  (cl-vec->marray (diagonal (cl-array matrix) :offset -1)))))
...
(defun d/e/f->tridiag (d e f)
  (let ((n (dim0 d)))
    ...
    (let ((matrix (make-marray 'double-float :dimensions (list n n)
			       :initial-element 0d0)))
      (with-_-indexing1 (((matrix 0 0)
			 (d 0)
			 (e 0)
			 (f 0))
			 maref)
...
(define-test tridiag-matrix-inverse
  (assert-true
   (identity-matrix-p 
    (labels ((crunch (test-matrix)
