(defpackage :tridiag-inverse
  (:use :cl :gsll :array-indexing :lisp-unit :anaphora :grid)
  (:import-from :iterate
		:iter :for))

(in-package :tridiag-inverse)
...
with theta_0 = 1, theta_1 = a_0"
  (let ((theta (gsll:make-marray 'double-float :dimensions (1+ N))))
    (with-_-indexing3 ((theta ((i-1 (- i 1))
			       (i-2 (- i 2)))
			      maref)
		       (a ((i (- i 1)))
			  maref)
